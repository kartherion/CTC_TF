    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            self.optimizer = tf.train.MomentumOptimizer(
                learning_rate=self.learning_rate, momentum=FLAGS.momentum, use_nesterov=True).minimize(self.cost, global_step=self.global_step)
        ...
        for cur_epoch in range(FLAGS.num_epochs):
            ...
            for cur_batch in range(num_batches_per_epoch):
                ...
                summary_str, batch_cost, step, _ = sess.run(
                    [g.merged_summay, g.cost, g.global_step, g.optimizer], feed)
                # calculate the cost
                ...
                if step % FLAGS.save_steps == 1:
                    ...
                    saver.save(
                        sess, os.path.join(FLAGS.checkpoint_dir, 'ocr-model'),
                        global_step=step)
                ...
                if step % FLAGS.validation_steps == 0:
                    ...
                    acc = utils.accuracy_calculation(
                        val_feeder.labels, dense_decoded, ignore_value=-1, isPrint=True)
                    ...
                    print(
                        log.format(
                            now.month, now.day, now.hour, now.minute, now.second, cur_epoch + 1,
                            FLAGS.num_epochs, acc, avg_train_cost, lastbatch_err, time.time() -
                            start_time, lr))
